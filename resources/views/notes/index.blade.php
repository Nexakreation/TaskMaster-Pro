...
<script>
    function stickyWall() {
        // const baseUrl = window.location.origin + '/taskmaster-pro/public';
        // OR using Laravel's URL helper:
        const baseUrl = '{{ url("taskmaster-pro/public") }}';
        ...
        return {
            ...
            async addNote() {
                try {
                    ...
                    alert('Failed to add note. Please check the console for details.');
                }
            },
            ...
            async drop(event, targetNote) {
                ...
                try {
                    await fetch(`${baseUrl}/api/notes/reorder`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            async updateNote() {
                ...
                try {
                    const response = await fetch(`${baseUrl}/api/notes/${this.editingNote.id}`, {
                        method: 'PATCH',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
    ...
    <div class="flex-1 p-2">
        <div class="max-w-7xl mx-auto">
            ...
            <!-- Notes Container with Tag Filter -->
            <div x-data="stickyWall()" class="space-y-6">
                <!-- Tag Filter -->
                <div class="flex flex-wrap gap-2 overflow-x-auto pb-2 -mx-2 px-2 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600">
                    <button @click="selectedTag = 'all'"
                        :class="{ 'bg-purple-600 text-white': selectedTag === 'all', 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300': selectedTag !== 'all' }"
                        class="px-4 py-2 rounded-full text-sm font-medium transition-colors whitespace-nowrap flex-shrink-0">
                        All Notes
                    </button>
                    <template x-for="tag in uniqueTags" :key="tag">
                        <button @click="selectedTag = tag"
                            :class="{ 'bg-purple-600 text-white': selectedTag === tag, 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300': selectedTag !== tag }"
                            class="px-4 py-2 rounded-full text-sm font-medium transition-colors whitespace-nowrap flex-shrink-0"
                            x-text="tag">
                        </button>
                    </template>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <!-- No Notes Message -->
                    <div x-show="filteredNotes.length === 0 && notes.length > 0"
